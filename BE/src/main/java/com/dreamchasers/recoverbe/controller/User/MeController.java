...
public class MeController {
    ...
    public ResponseEntity<ResponseObject> searchContact(@RequestParam String name) {
     ...
     @GetMapping("/{email}/followed")
    public ResponseEntity<ResponseObject> checkFollowed(@PathVariable String email) {
        var result = userService.checkFollowed(email);
        return ResponseEntity.status(result.getStatus()).body(result);
    }
    ...
    @PutMapping("/{email}/follow")
    public ResponseEntity<ResponseObject> follow(@PathVariable String email) {
        var result = userService.follow(email);
        return ResponseEntity.status(result.getStatus()).body(result);
    }

    @PutMapping("/{email}/un-follow")
    public ResponseEntity<ResponseObject> unFollow(@PathVariable String email) {
        var result = userService.unFollow(email);
        return ResponseEntity.status(result.getStatus()).body(result);
    }
...
//    }
    ...
    public ResponseEntity<ResponseObject> uploadAvatar(@RequestPart ("avatar") MultipartFile avatar) {
        var result = userService.upload(avatar);
        return ResponseEntity.status(result.getStatus()).body(result);
    }
    ...
    public ResponseEntity<ResponseObject> rateCourse(@PathVariable UUID courseId, @RequestBody Rating rating) {
        var result = courseService.updateRatingCourse(courseId, rating);
        return ResponseEntity.status(result.getStatus()).body(result);
    }
    ...
    @PostMapping(value = "/{email}/post/create", consumes = "application/json;charset=UTF-8")
