...
public class InstructorController {
    ...
    public ResponseEntity<ResponseObject> getStatistic() {
        var result = instructorService.getTotalStatistic();
        return ResponseEntity.status(result.getStatus()).body(result);
    }

    @GetMapping("/statistics/filter")
    public ResponseEntity<ResponseObject> getStatisticByFilter(@RequestParam String type, @RequestParam String dateRange, @RequestParam int page, @RequestParam int size) {
        var result = instructorService.getStatisticByFilter(type, dateRange, size, page);
        return ResponseEntity.status(result.getStatus()).body(result);
    }
    ...
    public ResponseEntity<ResponseObject> getInvoices(@RequestParam int page, @RequestParam int size, @RequestParam boolean isDeleted) {
        var result = invoiceService.getInvoices(page, size, isDeleted, false);
        return ResponseEntity.status(result.getStatus()).body(result);
    }
    ...
    public ResponseEntity<ResponseObject> searchByEmail(@RequestParam String email, @RequestParam int page, @RequestParam int size, @RequestParam boolean isDeleted) {
        var result = invoiceService.searchByEmail(email, false, page, size, isDeleted);
        return ResponseEntity.status(result.getStatus()).body(result);
    }
    ...
    @PutMapping("/invoices/{id}/restore")
    ...
    @DeleteMapping("/invoices/delete/soft/{id}")
