...
import com.dreamchasers.recoverbe.entity.User.*;
import com.dreamchasers.recoverbe.enums.*;
...
public class CourseService {
    ...
    public ResponseObject updateRatingCourse(UUID courseId, Rating rate) {
    ...
    public ResponseObject filterCourses(String title, List<UUID> categories, String sort, int page, int size) {
        BooleanBuilder builder = new BooleanBuilder();
        QCourse course = QCourse.course;
        JPAQuery<Course> query;

        if (title != null && !title.isEmpty()) {
            builder.and(course.title.containsIgnoreCase(title));
        }
        if (categories != null && !categories.isEmpty()) {
            builder.and(course.categories.any().id.in(categories));
        }

        if("featured".equalsIgnoreCase(sort)) {
            query = queryFactory.selectFrom(course).where(builder).orderBy(course.totalRating.desc());
        }
        else if("newest".equalsIgnoreCase(sort)) {
            query = queryFactory.selectFrom(course)
                    .where(builder).orderBy(course.createdAt.desc());
        }
        else if("oldest".equalsIgnoreCase(sort)) {
            query = queryFactory.selectFrom(course)
                    .where(builder).orderBy(course.createdAt.asc());
        }
        else if("price_asc".equalsIgnoreCase(sort)) {
            query = queryFactory.selectFrom(course)
                    .where(builder).orderBy(course.price.asc());
        }
        else if("price_desc".equalsIgnoreCase(sort)) {
            query = queryFactory.selectFrom(course)
                    .where(builder).orderBy(course.price.desc());
        }
        else {
            query = queryFactory.selectFrom(course).where(builder);
        }

        Long totalElements = queryFactory
                .select(course.count())
                .from(course)
                .where(builder.and(course.deleted.eq(false)))
                .fetchOne();

        List<Course> listCourse = query.limit(size).offset((long) page * size).fetch();

        var listCourseDTO = convertService.convertToListCourseDTO(listCourse);

        PageImpl<CourseDTO> pageResult = new PageImpl<>(listCourseDTO, PageRequest.of(page, size), totalElements);
...
//    }
    ...
    public ResponseObject adminUpdateCourseStatus(UUID courseId, StatusChangeDTO statusChangeDTO) {
        ...
        if(statusChangeDTO.getStatus() == CoursePostStatus.REJECTED) {
            ...
            notificationService.adminSendUpdateStatusCourseToUser(course, getCurrentUser(), NotificationType.COURSE_REJECTED, statusChangeDTO.getStatus());
        }
        else {
            course.setReasonReject(null);
            course.setStatus(statusChangeDTO.getStatus());
            notificationService.adminSendUpdateStatusCourseToUser(course, getCurrentUser(), NotificationType.COURSE_APPROVED, statusChangeDTO.getStatus());
        }
        courseRepository.save(course);

        sendNotificationToFollower(course, course.getAuthor());
    ...
    private List<User> getFollowers(User user) {
        return queryFactory.selectFrom(QUser.user)
                .where(QUser.user.followings.contains(user))
                .fetch();
    }


    public void sendNotificationToFollower(Course course, User user) {
        List<User> followers = getFollowers(user);


        followers.forEach(follower -> {
            var noti = Notification.builder()
                    .content("Course " + course.getTitle() + " has been published by " + user.getFullName())
