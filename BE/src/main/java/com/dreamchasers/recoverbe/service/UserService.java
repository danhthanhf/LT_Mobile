...
public class UserService {
    ...
    public ResponseObject getAll(String type, int page, int size) {
        ...
        List<Notification> notifications = queryFactory.selectFrom(notification).where(builder).orderBy(notification.createdAt.desc()).offset((long) page * size).limit(size).fetch();

        //get total current count
        long totalCount = Optional.ofNullable(queryFactory.select(notification.count()).from(notification).where(builder).fetchOne()).orElse(0L);

        //get total all element count
        long totalAllElements = Optional.ofNullable(queryFactory.select(notification.count()).from(notification).where(notification.recipient.email.eq(user.getEmail())).fetchOne()).orElse(0L);
    ...
    public ResponseObject checkFollowed(String email) {
        var user = getCurrentUser();

        var friend = userRepository.findByEmail(email).orElseThrow(() -> new EntityNotFoundException("User not found"));

        return ResponseObject.builder().status(HttpStatus.OK).content(user.getFollowings().contains(friend)).build();
    }

    public ResponseObject unFollow(String email) {
        var user = getCurrentUser();
        var friend = userRepository.findByEmail(email).orElseThrow(() -> new EntityNotFoundException("User not found"));
        if(user.getFollowings().remove(friend)) {
    ...
    public ResponseObject follow(String email) {
        var user = getCurrentUser();
        var friend = userRepository.findByEmail(email).orElseThrow(() -> new EntityNotFoundException("User not found"));
        user.getFollowings().add(friend);
        userRepository.save(user);
        return ResponseObject.builder().status(HttpStatus.CREATED).build();
    }
...
//    }
    ...
    public boolean isPostFavorite(String email, Post post) {
        ...
        return post.getFavoriteUsers().contains(user);
    }
    ...
    public ResponseObject enrollCourse(User user, UUID courseId) {
        ...
        Course course = enrollService.enrollCourse(user, courseId);
    ...
    public ResponseObject upload(MultipartFile file) {
        try {
            ...
            return ResponseObject.builder().status(HttpStatus.BAD_REQUEST).message("Error occurred when uploading").build();
        }
    }
