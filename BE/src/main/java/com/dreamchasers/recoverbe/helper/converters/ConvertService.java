...
public class ConvertService {
    ...
    public PageModel<UserBasicDTO> convertEnrollmentPageToUserBasicDTOPageModel(PageModel<Enrollment> enrollmentPageModel) {
        List<UserBasicDTO> userBasicDTOList = enrollmentPageModel.getContent().stream()
                .map(enrollment -> convertToUserBasicDTO(enrollment.getUser()))
                .toList();

        return PageModel.<UserBasicDTO>builder()
                .page(enrollmentPageModel.getPage())
                .size(enrollmentPageModel.getSize())
                .totalPages(enrollmentPageModel.getTotalPages())
                .totalElements(enrollmentPageModel.getTotalElements())
                .content(userBasicDTOList)
                .build();
    }

    public StatisticContent convertInvoiceToStatisticContent(Invoice invoice) {
        return StatisticContent.builder()
                .user(convertToUserBasicDTO(invoice.getUser()))
                .course(convertToCourseDTO(invoice.getCourse()))
                .revenue((int) invoice.getTotal())
                .createdAt(invoice.getCreatedAt())
                .build();
    }

    public StatisticContent convertEnrollmentToStatisticContent(Enrollment enrollment) {
            return StatisticContent.builder()
                .user(convertToUserBasicDTO(enrollment.getUser()))
                .course(convertToCourseDTO(enrollment.getCourse()))
                .createdAt(enrollment.getCreatedAt())
                .build();
    }

    public PageModel<CourseDTO> convertEnrollmentPageToCourseDTOPageModel(PageModel<Enrollment> enrollmentPageModel) {
        List<CourseDTO> courseDTOPage = enrollmentPageModel.getContent().stream()
                .map(enrollment -> convertToCourseDTO(enrollment.getCourse()))
                .toList();

        return PageModel.<CourseDTO>builder()
                .page(enrollmentPageModel.getPage())
                .size(enrollmentPageModel.getSize())
                .totalPages(enrollmentPageModel.getTotalPages())
                .totalElements(enrollmentPageModel.getTotalElements())
                .content(courseDTOPage)
                .build();
    }

    public PageModel<CourseDTO> convertInvoicePageToCourseDTOPageModel(PageModel<Invoice> invoicesPage) {
        List<CourseDTO> courseDTOPage = invoicesPage.getContent().stream()
                .map(i -> convertToCourseDTO(i.getCourse()))
                .toList();

        return PageModel.<CourseDTO>builder()
                .page(invoicesPage.getPage())
                .size(invoicesPage.getSize())
    ...
    public PageModel<UserBasicDTO> convertInvoicePageToUserBasicDTOPageModel(PageModel<Invoice> invoicePageModel) {
        List<UserBasicDTO> userBasicDTOList = invoicePageModel.getContent().stream()
                .map(invoice -> convertToUserBasicDTO(invoice.getUser()))
                .toList();

        return PageModel.<UserBasicDTO>builder()
                .page(invoicePageModel.getPage())
                .size(invoicePageModel.getSize())
                .totalPages(invoicePageModel.getTotalPages())
                .totalElements(invoicePageModel.getTotalElements())
                .content(userBasicDTOList)
                .build();
    }
    ...
    public NotificationDTO convertToNotificationDTO(Notification notification) {
        return NotificationDTO.builder()
                ...
                .reasonReject(notification.getReasonReject())
